# SANO SANO — Copywriting du site web

> *Skill activé : `copywriting`*
> *Brand voice : Festif sans excès. Élégant sans prétention. Confiant sans bruit.*
> *Mots-clés à utiliser : célébrer, bulles, botanique, vivant, dynamisation, sans compromis, festif, frais, conscient*
> *Mots à éviter : "alternative", "comme le champagne", "substitut", "sans alcool" (à utiliser avec parcimonie, jamais en hero)*

---

## 🍾 HOMEPAGE

### HERO (Above the fold)

**Headline :**
# Festif par nature.

**Subheadline :**
Basilic vif, gingembre qui claque, thym sauvage, fraise de saison — chaque gorgée, une architecture. La boisson botanique pétillante créée pour ceux qui mangent bien et boivent mieux.

---

### SECTION 1.5 — Les principes

**Zero alcool. Jamais fermenté. Botanique frais et low calories.**

---

### SECTION 2 — Le goût (Pleasure/USP)

**Header :**
## Trinquer, c'est un rituel. Pas une obligation de boire.

**Body :**
Certaines boissons se boivent. Celle-là se goûte. La bulle qui titille, le basilic qui s'ouvre sur le gingembre, la fraise qui fond sous le thym — une complexité aromatique qui n'a rien à envier à la carte des vins. Pensée par un chef : ça se sent à la première gorgée.

---

### SECTION 3 — La promesse (Solution/Benefits)

**Header :**
## Botanique. Pétillant. Irréprochable.

**Sous-header :**
## Complexe. Pur. Jamais fermenté.

**Bénéfices (4 colonnes) :**

| 🌿 **Complexité aromatique** | 🔬 **Dynamisée** | 💧 **Jamais fermentée** | ⚡ **Low Calories** |
|---|---|---|---|
| Deux assemblages botaniques développés comme des accords de cuisine. Pas une saveur, une séquence. | Pour que chaque plante exprime son caractère jusqu'en fond de verre. Aucun arôme ajouté. | Aucun compromis aromatique. La pureté botanique, intacte. 0,00 % absolu. | Parce qu'un grand goût n'a pas à peser lourd. |

---

### SECTION 4 — Les produits (Products)

**Header :**
## Deux bulles, Deux caracteres.

**Produit 1 — Bulles Blanches**
### Basilic × Gingembre
*Frais, vibrant, tonique.*

La verdeur du basilic en attaque, la chaleur du gingembre en finale. Extra brut, élégant, parfaitement équilibré.

`Découvrir Basilic – Gingembre →`

---

**Produit 2 — Bulles Rosées**
### Thym × Fraise
*Surprenant, désaltérant, tout en nuances.*

Le thym ouvre la dégustation. La fraise l'arrondit. Une composition complexe, ronde, légèrement fruitée.

`Découvrir Thym – Fraise →`

---

**CTA Section :**
`Choisissez votre bulle. Ou les deux. →`

---

### SECTION 5 — Le procédé (Trust & Differentiation)

**Header :**
## Une bulle pensée comme un grand effervescent. Sans alcool. Sans fermentation. Sans compromis.

**Sous-header :**
## La plante. Rien que la plante.

**Body :**
Nous sélectionnons des plantes fraîches à leur apogée aromatique. Un procédé artisanal respectueux capte leur essence intacte. Une filtration délicate la préserve jusqu'à votre verre.

Le résultat : la pureté d'une plante qui vient d'être cueillie, vivante en bulles.

`Voir comment c'est fait →`

---

### SECTION 6 — Social Proof / Origine

**Header :**
## Né à Foster Farm.<br>Première Slow Food Farm de Belgique.

**Body :**
Aux portes de Bruxelles, un écosystème agricole régénératif. Eau de source. Énergie verte. Ingrédients 100% bio, tracés, certifiés.

Une promesse simple : respect du vivant, valorisation des plantes, naturalité absolue.

**Logos de validation** : Bio Garantie Belgique · FR-BIO-01 · Slow Food

---

### SECTION 7 — Moments / Use Cases

**Header :**
## Pour les moments qui comptent.

**3 colonnes visuelles :**

🥂 **L'apéritif haut de gamme**
Quand tu veux trinquer sans subir.

🍽️ **Le repas conscient**
Quand le plat mérite plus qu'une eau pétillante.

🎉 **La célébration**
Quand chacun lève son verre — sans exclusion.

---

### SECTION 8 — Où acheter (Map interactive)

**Header :**
## SANO SANO près de chez toi.

**Sous-titre :**
En ligne, en boutique, ou dans tes restaurants préférés. Choisis ton mode de vie.

**[MAP INTERACTIVE - Google Maps / Mapbox]**

**Points de vente – Choisis ta manière d'acheter :**

| 🏪 **En ligne** | 🍇 **En boutique** | 🍽️ **En restaurant** |
|---|---|---|
| sanosanodrinks.com<br>Livraison rapide | Cavistes sélectionnés<br>Épiceries fines<br>Magasins bio | Restaurants partenaires<br>Hôtels<br>Bars à vin<br>Traiteurs |

**Filtre la map par type :**
- [x] En ligne
- [x] Points de vente physiques  
- [x] Restaurants/HoReCa

**Ou cherche par code postal :**
`[Champ de saisie CP]` → `Chercher à proximité`

---

### SECTION 9 — Final CTA

**Header :**
## Prêt à célébrer autrement ?

**Body :**
75 cl. Bouchon liège. Muselet. À partir de 9,90 €.

**CTAs :**
- `🗺️ Trouver près de chez moi` (scroll to map)
- `📦 Commander en ligne — 9,90 €` (vers boutique)
- `🎁 Pack découverte (2 bouteilles) — 18,90 €`

**Reassurance :** Livraison gratuite en Belgique dès 50 € · Paiement sécurisé · Conservé bio

---
---

## 🌿 PAGE PRODUIT — BULLES BLANCHES (BASILIC × GINGEMBRE)

### HERO

**Headline :**
# Basilic × Gingembre.<br>L'attaque verte, la finale chaude.

**Subheadline :**
Bulles blanches. Extra brut. 0,00 % alcool. La verdeur du basilic relevée par la tension épicée du gingembre — pour un apéritif vivant, tendu, mémorable.

**Image :** Bouteille en gros plan, verre servi à côté, herbes fraîches en arrière-plan.

**CTA :** `Ajouter au panier — 9,90 €`

---

### SECTION — Profil aromatique

**Header :**
## La dégustation en trois temps.

**Body (storytelling) :**

**Attaque** — Le basilic se déploie d'abord. Vert, frais, herbacé. C'est l'arôme que tu reconnais avant même de boire.

**Milieu de bouche** — La bulle fine arrive, structurée, sèche. Aucun sucre pour masquer.

**Finale** — Le gingembre prend le relais. Une pointe épicée, longue, tonique. La sensation reste — fraîche et nette.

---

### SECTION — Accord parfait

**Header :**
## À servir avec…

- 🥗 Salades vertes, tartares de poisson, ceviche
- 🍣 Sushis, sashimis, cuisine japonaise
- 🧀 Fromages frais (chèvre, ricotta, mozzarella)
- 🌶️ Plats légèrement épicés (thaï, indien doux)
- 🍓 Apéritifs classiques (olives, anchois, tapenade)

**Température idéale :** 6-8°C, dans une flûte ou un verre à vin blanc.

---

### SECTION — Composition

**Header :**
## Tout. Et rien d'autre.

**Ingrédients :**
Eau · Basilic* · Miel d'acacia* · Gingembre* · Acide ascorbique · Acide citrique
*\*100% biologiques*

**Valeurs nutritionnelles (100ml) :**
| Énergie | Glucides | Sucres | Sel | Alcool |
|---------|----------|--------|-----|--------|
| 29 kJ / 7 kcal | 0,9 g | 1,5 g | 0,01 g | **0,00 %** |

✓ Sans gluten · Sans sulfites · Bio · Végétarien

---

### CTA FINAL

**Header :**
## Une bouteille. Six verres. Zéro lendemain.

`Commander — 9,90 €` `Pack découverte (2 bouteilles) — 18,90 €`

---
---

## 🌸 PAGE PRODUIT — BULLES ROSÉES (THYM × FRAISE)

### HERO

**Headline :**
# Thym × Fraise.<br>L'herbe ouvre, le fruit referme.

**Subheadline :**
Bulles rosées. Extra brut. 0,00 % alcool. La note herbacée du thym, arrondie par la douceur acidulée de la fraise — une rondeur sèche, surprenante, désaltérante.

**Image :** Bouteille rose pâle, verre servi, thym et fraises fraîches.

**CTA :** `Ajouter au panier — 9,90 €`

---

### SECTION — Profil aromatique

**Header :**
## Une dégustation surprenante.

**Body :**

**Attaque** — Le thym entre en premier. Herbe sèche, méditerranéenne, racée. On ne s'y attend pas dans une boisson rosée.

**Milieu de bouche** — Les bulles fines portent la fraise. Acidulée, légère, jamais sucrée.

**Finale** — Les deux dansent. Une rondeur sèche qui revient, longue, désaltérante.

---

### SECTION — Accord parfait

**Header :**
## À servir avec…

- 🐟 Poissons grillés, crustacés, plateaux fruits de mer
- 🍅 Cuisine méditerranéenne, tomates, basilic
- 🍰 Desserts peu sucrés aux fruits rouges
- 🦆 Carpaccios de bœuf, jambons crus, charcuteries fines
- 🌅 Apéritif d'été, terrasse, brunch

**Température idéale :** 6-8°C, dans une flûte ou un verre tulipe.

---

### SECTION — Composition

**Ingrédients :**
Eau · Fraise* · Framboise* · Miel d'acacia* · Thym* · Acide ascorbique · Acide citrique
*\*100% biologiques*

**Valeurs nutritionnelles (100ml) :**
| Énergie | Glucides | Sucres | Sel | Alcool |
|---------|----------|--------|-----|--------|
| 33 kJ / 8 kcal | 0,9 g | 1,5 g | 0,01 g | **0,00 %** |

---

### CTA FINAL

`Commander — 9,90 €` `Pack découverte (2 bouteilles) — 18,90 €`

---
---

## 🏡 PAGE — NOTRE HISTOIRE

### HERO

**Headline :**
# Née en Belgique, d'une conviction simple.

**Subheadline :**
On peut célébrer sans se mentir. Sans s'effacer. Sans accepter une eau parce qu'on a choisi de ne pas boire.

---

### SECTION 1 — Le point de départ

**Header :**
## Le constat qui nous a réunis.

**Body :**

Il fallait choisir entre deux mondes :

Celui des bulles festives, qui contiennent ce qu'on ne veut plus — alcool, sulfites, sucres ajoutés, fermentation.

Et celui des "alternatives" — soft drinks sucrés, jus, eaux aromatisées. Aucun n'avait la complexité, l'élégance, la tenue d'un grand effervescent.

Entre les deux, un vide. Nous l'avons rempli.

---

### SECTION 2 — Notre vision

**Header :**
## Pas une imitation. Une nouvelle expérience.

**Body :**

SANO SANO n'est ni un champagne sans alcool, ni un soft drink premium. C'est une **catégorie en soi** : la bulle botanique dynamisée, pour adultes exigeants.

Notre matière première, ce sont les plantes — fraîches, biologiques, sélectionnées à leur apogée. Notre signature, c'est la dynamisation — un procédé qui en extrait la pureté sans la dénaturer.

Le reste — bulle fine, structure extra brute, finale longue — c'est l'écho d'une exigence : que la dégustation soit aussi belle que le rituel qui l'entoure.

---

### SECTION 3 — Foster Farm

**Header :**
## Là où nous produisons.

**Body :**

Foster Farm, aux portes de Bruxelles. Première **Slow Food Farm** de Belgique. Un écosystème agricole régénératif, alimenté à l'énergie verte, à l'eau de source.

Chaque lot est tracé, certifié bio. Chaque plante est cueillie au plus près, à son apogée. Production artisanale, à petite échelle, pour préserver l'intégrité aromatique.

Ce n'est pas une usine. C'est un jardin productif.

---

### SECTION 4 — La promesse

**Header :**
## Trois engagements. Tenus.

| 🌱 **Fraîcheur** | 🤲 **Respect** | ✨ **Élégance** |
|---|---|---|
| Plantes cueillies à leur apogée, traçabilité bio totale. | Du sol à la bouteille — agriculture régénérative, énergie verte, savoir-faire artisanal. | Verre, bouchon liège, muselet. Un packaging à la hauteur du contenu. |

---

### CTA

**Header :**
## Goûter, c'est comprendre.

`Découvrir la gamme →`

---
---

## 🔬 PAGE — LE PROCÉDÉ (DYNAMISATION)

### HERO

**Headline :**
# La dynamisation.<br>Notre façon de capturer le vivant.

**Subheadline :**
Pas de fermentation. Pas de chaleur destructrice. Pas d'arômes ajoutés. Juste la plante, exprimée à son maximum.

---

### SECTION — Le procédé en 3 étapes

**Header :**
## Comment ça marche.

**Étape 1 — Sélection**
Nous choisissons chaque plante au moment exact où ses arômes sont à leur sommet. Basilic en début de matinée. Thym en fin de floraison. Gingembre frais, jamais déshydraté.

**Étape 2 — Infusion ultrasonique**
Les plantes sont infusées par ultrasons dans une eau de source pure. Cette technique extrait les composés aromatiques **sans chauffer**, sans les dénaturer. Ce qu'on goûte ensuite, c'est exactement ce que la plante contenait à la cueillette.

**Étape 3 — Filtration délicate**
Une filtration douce préserve la finesse aromatique tout en garantissant la pureté du produit final. Puis la bulle est intégrée — fine, structurée, persistante.

---

### SECTION — Pourquoi ça change tout

**Header :**
## Ce que la dynamisation rend possible.

**Tableau comparatif :**

| | **SANO SANO** | Vins désalcoolisés | Soft drinks premium | Kombucha |
|---|---|---|---|---|
| Alcool | **0,00 %** | 0,3 - 0,5 % résiduel | 0 % | Trace (~0,5 %) |
| Fermentation | **Jamais** | Oui puis désalcoolisé | Souvent | Oui |
| Arômes | **Plantes fraîches uniquement** | Souvent ajoutés post-désalcoolisation | Concentrés / sirops | Naturels mais acides |
| Sucres | **1,5 g / 100ml** | 4-12 g / 100ml | 8-12 g / 100ml | 2-5 g / 100ml |
| Calories | **7-8 kcal** | 20-50 kcal | 30-50 kcal | 15-25 kcal |
| Bio | **100 %** | Variable | Variable | Variable |

---

### SECTION — La signature

**Header :**
## Le résultat, dans le verre.

**Body :**

Une attaque franche. Une bulle fine et persistante. Une structure extra brute, sans la moindre rondeur sucrée. Une longueur en bouche qui rappelle celle d'un grand effervescent.

Et derrière tout cela — le goût pur de la plante, intact.

---

### CTA

`Voir nos bulles →`

---
---

## 🛍️ PAGE — BOUTIQUE

### HERO

**Headline :**
# Choisissez votre célébration.

**Subheadline :**
Bouteille à l'unité, pack découverte, ou abonnement SANO Club — pour toujours en avoir au frais.

---

### SECTION — Produits

**Cards Produits (Grid 3 colonnes) :**

**Carte 1 :**
**Bulles Blanches — Basilic × Gingembre**
75 cl · 6 verres · 0,00 % alcool
**9,90 €**
`Ajouter au panier`

**Carte 2 :**
**Bulles Rosées — Thym × Fraise**
75 cl · 6 verres · 0,00 % alcool
**9,90 €**
`Ajouter au panier`

**Carte 3 :**
**Pack Découverte — 2 bouteilles**
1 Blanches + 1 Rosées
**18,90 €** ~~19,80 €~~
`Ajouter au panier`

---

### SECTION — SANO Club

**Header :**
## SANO Club. Toujours en bulles.

**Body :**
Recevez 4 bouteilles chaque mois (mix Blanches & Rosées, ou personnalisé). Livraison gratuite. Annulation libre à tout moment.

**À partir de 35 € / mois**

`Rejoindre le Club →`

---

### SECTION — Réassurance

| 🚚 **Livraison gratuite** | 🔒 **Paiement sécurisé** | ↩️ **Satisfait ou remboursé** | 🌍 **Belgique + Europe** |
|---|---|---|---|
| Dès 50 € en Belgique | Stripe / Bancontact / Apple Pay | 14 jours | Livraison sous 3-5 jours |

---
---

## 🍽️ PAGE — B2B (HORECA / PROFESSIONNELS)

### HERO

**Headline :**
# Pour les tables qui n'oublient personne.

**Subheadline :**
Restaurants étoilés, hôtels, traiteurs, cavistes — ajoutez à votre carte la première bulle botanique dynamisée. Un choix qui respecte tous vos clients, sans compromis sur le standing.

**CTA :** `Demander un échantillon →`

---

### SECTION — Pourquoi SANO SANO sur votre carte

**3 colonnes :**

🍾 **Une vraie offre sans alcool**
Plus jamais "un jus d'orange ou une eau pétillante" pour vos clients qui ne boivent pas. Une bouteille qui se sert à la flûte, comme un grand cru.

📈 **Un segment en croissance**
+30% / an sur le sans alcool premium. Femmes enceintes, sober curious, conducteurs désignés — un public qui dépense, et qui revient.

🌟 **Une promesse cohérente**
Bio, artisanal, belge, traçable. Un produit aligné avec les valeurs de vos clients exigeants.

---

### SECTION — Conditions pro

**Header :**
## Travailler avec nous.

| Format | Carton | Prix HT | Disponibilité |
|--------|--------|---------|--------------|
| 75 cl bouteille | 6 / carton | Sur demande | Stock permanent |
| 75 cl bouteille | 12 / carton | Sur demande | Stock permanent |

**Marges pro · Formation sommellerie · PLV offerte · Livraison Belgique + France**

---

### SECTION — Ils nous servent

**Header :**
## Déjà sur les cartes de…

*[Logos restaurants partenaires — à compléter]*

> *"Enfin une bulle sans alcool que je n'ai pas honte de proposer en accord avec mes plats."*
> — Sommelier partenaire, restaurant étoilé

---

### CTA FINAL

**Header :**
## Recevez un échantillon gratuit.

**Body :**
Une bouteille de chaque variété, livrée à votre établissement, pour la déguster en équipe.

**Formulaire court :**
- Nom de l'établissement
- Type d'établissement (restaurant, hôtel, caviste, traiteur, autre)
- Adresse de livraison
- Email professionnel

`Recevoir mes échantillons →`

**Ou contactez :**
**Jérôme Goffinet** — +32 475 95 46 33 — jerome@osandrinks.com

---
---

## 📩 PAGE — CONTACT

### HERO

**Headline :**
# Une question ? On répond.

**Subheadline :**
Particulier, professionnel, presse — choisissez votre canal.

---

### SECTION — Trois canaux

**Card 1 — Particuliers**
📦 Commandes, livraisons, SANO Club
*hello@sanosanodrinks.com*

**Card 2 — Professionnels**
🏨 HoReCa, cavistes, distribution
**Jérôme Goffinet**
+32 475 95 46 33
jerome@osandrinks.com

**Card 3 — Presse**
📰 Interviews, échantillons presse, médias
press@sanosanodrinks.com

---

### SECTION — Notre adresse

**OSAN SRL**
Foster Farm — ch. de Vieusart 35
1300 Wavre · Belgique

---
---

## 🔍 META CONTENT (SEO)

### Homepage
**Title :** `SANO SANO — Bulles botaniques 0,00 % alcool · Bio · Belge`
**Meta :** `Célébrez sans une goutte. SANO SANO, la première bulle botanique dynamisée — 0,00 % alcool, bio, low calories. Basilic-Gingembre & Thym-Fraise. Made in Belgium.`

### Page Bulles Blanches
**Title :** `SANO SANO Basilic × Gingembre — Bulles blanches sans alcool bio`
**Meta :** `Bulles blanches dynamisées au basilic et gingembre bio. 0,00 % alcool, extra brut, 7 kcal/100ml. Pour célébrer sans compromis. 9,90 €.`

### Page Bulles Rosées
**Title :** `SANO SANO Thym × Fraise — Bulles rosées sans alcool bio`
**Meta :** `Bulles rosées dynamisées au thym et à la fraise bio. 0,00 % alcool, extra brut, 8 kcal/100ml. Désaltérantes, surprenantes. 9,90 €.`

### Notre Histoire
**Title :** `Notre histoire — SANO SANO, née à Foster Farm, Belgique`
**Meta :** `Née en Belgique de l'envie de célébrer sans compromis. SANO SANO est produite à Foster Farm, première Slow Food Farm de Belgique.`

### Procédé
**Title :** `La dynamisation — Notre procédé exclusif sans fermentation`
**Meta :** `Découvrez la dynamisation : infusion ultrasonique de plantes fraîches bio, sans fermentation, sans alcool, sans arômes ajoutés. La signature SANO SANO.`

### Boutique
**Title :** `Boutique SANO SANO — Bulles botaniques bio · Livraison Belgique`
**Meta :** `Commandez les bulles SANO SANO en ligne. Bouteille 9,90 €, pack découverte 18,90 €, abonnement SANO Club. Livraison gratuite dès 50 €.`

### B2B
**Title :** `SANO SANO Pro — Bulles sans alcool pour restaurants & cavistes`
**Meta :** `Ajoutez SANO SANO à votre carte. Bulles botaniques 0,00 % alcool pour restaurants, hôtels, cavistes, traiteurs. Échantillon gratuit sur demande.`

---
---

## 🎨 NOTES POUR L'ÉQUIPE WEB / DESIGN

### Tone & Voice — récap
- **Affirmé sans agressivité** — on ne se compare pas, on existe
- **Précis sans technique** — la dynamisation expliquée simplement
- **Festif sans bling** — l'élégance contemporaine, pas le luxe ostentatoire
- **Inclusif sans paternalisme** — on parle à des adultes qui ont déjà choisi

### Mots à utiliser systématiquement
*célébrer · bulles · botanique · vivant · dynamisation · plantes fraîches · sans compromis · pur · vif · contemporain · adulte · conscient · 0,00 %*

### Mots à BANNIR
*"sans alcool" (en hero) · "alternative" · "comme du champagne" · "substitut" · "healthy" · "detox" · "light" · "mocktail" · "fake" · "imitation"*

### Visuels recommandés
- Photos lifestyle élégantes — mains, verres, lumière naturelle (pas de "perfection" stérile)
- Plantes fraîches en gros plan
- Foster Farm — l'écosystème, pas l'usine
- Pas de visuels "santé-bien-être", on n'est pas une marque wellness
- Pas de mise en scène "fête bruyante", on est plus *dîner sur la terrasse*

### Hierarchy CTA
1. **CTA #1** — `Commander` / `Découvrir`
2. **CTA #2** — `Voir le procédé` / `En savoir plus`
3. **CTA #3** (B2B/secondary) — `Demander un échantillon`

---

## 🚀 PROCHAINES ÉTAPES SUGGÉRÉES

1. **A/B tester les 3 headlines** de la homepage (skill `ab-test-setup`)
2. **Créer la séquence email post-achat** (skill `email-sequence`) — 5 emails pour transformer 1ère commande en abonné SANO Club
3. **Optimiser le checkout** (skill `signup-flow-cro` / `page-cro`)
4. **Générer les visuels** (avec le MCP Higgsfield — `generate_image`)
5. **Créer le brief sommellerie B2B** (PDF/PLV pour restaurants)
